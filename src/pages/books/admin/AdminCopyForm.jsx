...
function buildBarcodeOptions(copies) {
  ...
  if (matchedBarcodes.length === 0) {
    return Array.from({ length: 10 }, (_, index) => `BC${String(index + 1).padStart(6, '0')}`)
  }
  ...
  return Array.from({ length: 10 }, (_, index) => `${selectedPrefix}${String(maxValue + index + 1).padStart(digitWidth, '0')}`)
}
  ...
  useEffect(() => {
    ...
    async function loadData() {
      try {
        ...
        const nextBooks = booksResponse.data?.content || []
        const nextCopies = copiesResponse.data?.content || []

        if (ignore) return
        setBooks(nextBooks)
        setBarcodeOptions(buildBarcodeOptions(nextCopies))
    ...
    <AdminLayout
      ...
      description={
        isEdit
          ...
          : 'Tạo thêm bản sao mới và lưu trực tiếp xuống backend.'
      }
    >
        ...
        <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-[0_12px_40px_-28px_rgba(15,23,42,0.35)]">
          ...
          <div className="p-5">
            ...
            <div className="grid gap-5 lg:grid-cols-2">
              <Field label="Mã vạch" required>
                {isEdit ? (
                  ...
                  <SelectInput value={form.barcode} onChange={(event) => updateField('barcode', event.target.value)}>
            ...
            <div className="mt-5 grid gap-5 lg:grid-cols-[1fr_260px]">
              <Field label="Đầu sách" required={!isEdit} hint={isEdit ? 'Đầu sách được giữ nguyên khi cập nhật bản sao.' : 'Chọn đầu sách sẽ sở hữu bản sao này.'}>
