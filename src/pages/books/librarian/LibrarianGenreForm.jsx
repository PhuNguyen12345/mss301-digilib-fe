...
function LibrarianGenreForm({ mode = 'add' }) {
  ...
  useEffect(() => {
    ...
    async function loadCategory() {
      if (!isEdit || !genreId) return

      try {
        const response = await getCategoryById(genreId)
  ...
  const categoryCode = useMemo(() => {
    if (!genreId) return 'Tự động tạo sau khi lưu'
    return `CAT-${String(genreId).padStart(4, '0')}`
  }, [genreId])
  ...
  async function handleSubmit(event) {
    ...
    try {
      ...
      if (isEdit && genreId) {
        await updateCategory(genreId, payload)
      } else {
        await createCategory(payload)
      }

      navigate('/librarian/genres')
    } catch (error) {
  ...
  return (
    <LibrarianLayout
      active="genres"
      title={isEdit ? 'Chỉnh sửa danh mục thể loại' : 'Thêm danh mục thể loại mới'}
      description={
        isEdit
          ? 'Cập nhật nhóm thể loại sách trực tiếp từ backend với giao diện đồng bộ cùng admin.'
          : 'Tạo danh mục thể loại mới và lưu trực tiếp xuống backend.'
      }
    >
      <form onSubmit={handleSubmit}>
        <section className="rounded-2xl border border-slate-200 bg-white p-5 shadow-[0_12px_40px_-28px_rgba(15,23,42,0.35)]">
          ...
          <div className="mt-6 flex justify-end gap-3 border-t border-slate-200 pt-5">
            <Link to="/librarian/genres" className="inline-flex h-11 items-center justify-center gap-2 rounded-2xl border border-slate-300 px-5 text-sm font-medium text-slate-700">
